...
sets_to_save = [("train", X_train, y_train), ("val", X_val, y_val), ("test", X_test, y_test)]
...
def augment_inplace(processed_base_path, subset='train', augment_probability=0.5):
    ...
    if not os.path.isdir(subset_path): return
    ...
    for label in os.listdir(subset_path):
        ...
        for fname in os.listdir(label_dir):
            if fname.lower().endswith(('.jpg', '.jpeg', '.png')):
                ...
                if random.random() > augment_probability: continue
                try:
